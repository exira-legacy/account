namespace AccountService.Structurizr
{
    using System;
    using System.IO;
    using System.Linq;
    ...
    public class Program
    {
        ...
        public static class Ids
        {
            ...
            public static int SoftwareSystemAccountService = 10001;
        ...
        private static readonly string SoftwareSystemAccountServiceId = Ids.SoftwareSystemAccountService.ToString();
        ...
        private static void Main()
        {
            ...
            var workspace = new Workspace("AccountService", "Voorbeeld register.")
            {
            ...
            var accountService = CreateSystemAccountService(model);

            var api = CreateContainerApi(accountService);
            var apiRunner = CreateContainerApiRunner(accountService);
            var apiStore = CreateContainerApiStore(accountService);
            var eventStore = CreateContainerEventStore(accountService);

            user.Uses(accountService, "raadpleegt", "HTTPS").AddTags(CustomTags.Https);
            ...
            adrToolsImporter.ImportArchitectureDecisionRecords(accountService);
        ...
        private static SoftwareSystem CreateSystemAccountService(Model model)
        {
            var accountService = model
                .AddSoftwareSystem(
                    Location.Internal,
                    "AccountService",
                    "Het `voorbeeld register laat gebruikers toe alle authentieke gegevens van een voorbeeld te raadplegen.");

            accountService.Id = SoftwareSystemAccountServiceId;
            accountService.Url = "https://github.com/exira/account-service";

            return accountService;
        }

        private static Container CreateContainerApi(SoftwareSystem accountService)
        {
            var api = accountService
                .AddContainer(
                    "Loket API",
        ...
        private static Container CreateContainerApiRunner(SoftwareSystem accountService)
        {
            var apiRunner = accountService
                .AddContainer(
        ...
        private static Container CreateContainerApiStore(SoftwareSystem accountService)
        {
            var apiStore = accountService
                .AddContainer(
                    "Loket API gegevens",
        ...
        private static Container CreateContainerEventStore(SoftwareSystem accountService)
        {
            var eventStore = accountService
                .AddContainer(
                    "Eventstore",
        ...
        {
            var accountService = model.GetSoftwareSystemWithId(SoftwareSystemAccountServiceId);

            var apiStore = accountService.GetContainerWithId(ContainerApiStoreId);
            var eventStore = accountService.GetContainerWithId(ContainerEventStoreId);
        ...
        private static void CreateContextView(ViewSet views, Model model)
        {
            ...
            contextView.Add(accountService);
        ...
        private static void CreateApiContainerView(ViewSet views, Model model)
        {
            ...
            var user = model.GetPersonWithName(PersonUserName);
            var api = accountService.GetContainerWithId(ContainerApiId);
            var apiStore = accountService.GetContainerWithId(ContainerApiStoreId);
            var eventStore = accountService.GetContainerWithId(ContainerEventStoreId);
        ...
        private static void CreateApiRunnerContainerView(ViewSet views, Model model)
        {
            ...
            var apiRunner = accountService.GetContainerWithId(ContainerApiRunnerId);
            var apiStore = accountService.GetContainerWithId(ContainerApiStoreId);
            var eventStore = accountService.GetContainerWithId(ContainerEventStoreId);
